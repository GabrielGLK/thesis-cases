...
static void sweep_x (scalar c, scalar cc)
{
  ...
  if (tracers) {
	...
	/**
	 In case of mixed cells, to calculate the concentration value $xf_j$ we have to divide
	 $t_j$ with the effective volume fraction occupied by the fluid, cs[]. 
	 */
...
#else
  vof_update_x (c, cc, flux, tracers, tfluxl, u_l, dt); 
#endif
  boundary ({c});
  boundary (tracers);
