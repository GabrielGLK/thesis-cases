...
8<x&&x<xx&&(y>R_ex?sq(x-8)+sq(y-R_0+R_out)>sq(R_out):y-R_ex>0)?top_lower:
...
refine (8<x&&x<xx&&(sq(x-8)+sq(y-R_0+R_out)<=sq(R_out))&&(sq(x-8)+sq(y-R_0+R_out)<=sq(R_out))&& level < maxlevel);
