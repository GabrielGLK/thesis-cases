...
#define xx (9-sqrt(2*(ratio-1)*R_0*R_out - sq(delta_length)))
...
// computational domain
  mask((x<=xx&&(y-R_0>0))?top_lower:
xx<x&&x<9&&(y>R_0?sq(x-9)+sq(y-R_ex+R_out)>sq(R_out):y-R_0>0)?top_lower:
(x>=xx&&(y-R_ex>0))?top_upper:
none);

// mesh refine initallly for interested region
refine ((x<=xx&&(y-R_0<0)) && level < maxlevel);
refine ((x>xx&&(y-R_ex<0)) && level < maxlevel);
refine (xx<x&&x<9&&(sq(x-9)+sq(y-R_ex+R_out)<=sq(R_out))&&(sq(x-9)+sq(y-R_ex+R_out)<=sq(R_out))&& level < maxlevel);
