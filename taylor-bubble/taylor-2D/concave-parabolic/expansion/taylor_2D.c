...
#define xx (8+sqrt(2*(ratio-1)*R_0*R_out - sq(delta_length)))
...
mask((x<=8&&(y-R_0>0))?top_lower:
8<x&&x<(8+delta_x)&&(sq(x-8)+sq(y-R_0 - R_out)<sq(R_out))?top_lower:
(x>8&&(y-R_ex>0))?top_lower:
none);

// mesh refine initallly for interested region
refine ((x<=8&&(y-R_0<0)) && level < maxlevel);
refine ((x>8&&(y-R_ex<0)) && level < maxlevel);
refine (8<x&&x<(8+delta_x)&&(sq(x-8)+sq(y-R_0 - R_out)<=sq(R_0-0.1))&&(sq(x-8)+sq(y-R_0 - R_out)>=sq(R_out))&& level < (maxlevel+2));
