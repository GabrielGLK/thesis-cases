...
// particle methods to track liquid trajectories
#define PARTICLE 1
...
#define delta_ratio (1 - ratio)
#define delta_length delta_ratio*R_0
#define theta degree*pi/180
#define R_out (delta_length)/(sq(sin(theta))*2)
#define delta_x delta_length/tan(theta)
#define xx (8+sqrt(2*(1-ratio)*R_0*R_out - sq(delta_length)))
...
mask((x<=(8+delta_x)&&(y-R_0>0))?top_lower:
8<x&&x<(8+delta_x)&&(y>R_ex?sq(x-(8+delta_x))+sq(y-R_ex-R_out)<sq(R_out):y-R_ex>0)?top_lower:
(x>=(8+delta_x)&&(y-R_ex>0))?top_upper:
none);

// mesh refine initallly for interested region
refine ((x<=(8+delta_x)&&(y-R_0<0)) && level < maxlevel);
refine ((x>(8+delta_x)&&(y-R_ex<0)) && level < maxlevel);
refine (8<x&&x<(8+delta_x)&&(sq(x-(8+delta_x))+sq(y-R_ex-R_out)<=sq(R_out))&&(sq(x-(8+delta_x))+sq(y-R_ex-R_out)<=sq(R_out))&& level < maxlevel);
