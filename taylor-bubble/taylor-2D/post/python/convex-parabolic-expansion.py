...
while i < 5:  
    try:
        PATH_REFERENCE = os.path.abspath('/media/lguo/Elements/calculation/Taylor_bubble/2D/singularity/parabolic/convex-parabolic/expansion/{}/{}/taylor_2D'.format(str(dir_list[a]),str(case[i])))
        ...
        PATH_COM = os.path.abspath("/media/lguo/Elements/writing/thesis-part/chapter-5/parabolic-convex/expansion/convex/figure")
        ...
        def get_sur_2(time,distance):
            ...
            with open(fn, 'r') as file:
                for line in file.readlines():
                    ...
                    l = line.split(' ')
                    #print float(l[0]), float(l[1]),float(l[2]),float(l[3])
                    arrs.append([float(l[0])-distance, float(l[1]),float(l[2]),float(l[3])])
            return arrs
        ...
        def simulation(a):
            ...
            time_begin = [11.6,3.2,9.4,7.8,10.5,11.1,11.3,11.5]
            time_end = 30
            dist = [8.76,6.425,8.15,7.7,8.456,8.623,8.679,8.734]
            distance = dist[i]
    

            for time in np.arange(time_begin[i],time_end,1):
                ...
                with open('data-{}'.format(i), 'aw') as f:  
                    f.write("%.1f %f %.4f\n" %(time, xh1, xh11))
                le1 = plot_sur(plt, sur1, distance, "r")
                ...
                if y_min  < 0:
                    y1 = math.fabs(y_min-(1 - delta_x) + dist[i] - 8 )/(y_max - y_min)
                    y_2 = math.fabs(y_min - 1 + dist[i] - 8)/(y_max - y_min)
                elif y_min < 1 and y_min > 0:
                    y1 = 0
                    y_2 = math.fabs(y_min- 1 + dist[i] - 8 )/(y_max - y_min)
                else:
                    y1 = 0
                ...
                # calculate the arc
                mxmy = [R_ex - R_out, (9 - xx-dist[i]+8)]
                mx1my1 =  [-R_ex + R_out, (9 - xx-dist[i]+8)]
                width = 2 * R_out
                height = 2 * R_out
                width_1 = 2*R_out
                height_1 = 2* R_out
            ...
            # plt.grid(True)
                ...
                name =  "basilisk_" + "%g"% (4*(time- time_begin[i]))
