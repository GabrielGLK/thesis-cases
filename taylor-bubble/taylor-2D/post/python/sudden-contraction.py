...
while i < 1:  
    try:
        PATH_REFERENCE = os.path.abspath('/media/lguo/Elements1/calculation/Taylor_bubble/2D/singularity/sudden/contraction/contraction_new/{}/{}-{}/taylor_2D'.format(str(dir_list[a]),str(dir_list[a]),str(case[i])))
        ...
        PATH_COM = os.path.abspath("/media/lguo/Elements1/writing/thesis-part/chapter-4/contraction/figure")
        ...
        def simulation(a):
            time_begin = [11.6,3.5,9.4,7.8,10.5,11.1,11.3,11.5]
            time_end = 30
            ...
            for time in np.arange(time_begin[i],time_end,1):
                ...
                ex= [0.93,0.81,0.69] # expansion ratio
                ...
                plt.axvline(x = -0.5*ex_ratio,ymin = y1, ymax = 1, color='b')
                #plt.axhline(y = 0,xmin=0.08*0.55/expansion,xmax=(expansion*0.55-0.5)/expansion,color='b')
                #plt.axhline(y = 0,xmin=0.5-0.07*0.5,xmax=0.5,color='b')
