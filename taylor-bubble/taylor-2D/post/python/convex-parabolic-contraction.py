...
while i < 2:  
    try:
        PATH_REFERENCE = os.path.abspath('/media/lguo/Elements1/calculation/Taylor_bubble/2D/singularity/parabolic/convex-parabolic/contraction/{}/{}/taylor_2D'.format(str(dir_list[a]),str(case[i])))
        ...
        PATH_COM = os.path.abspath("/media/lguo/Elements1/calculation/Taylor_bubble/2D/singularity/parabolic/convex-parabolic/figure/contraction")
        ...
        def simulation(a):
            sixty = {'1':7.4,'2':6,'3':5.2,'4':6.2,'5':5.8,'6':9.2,'7':10.8}
            seventy = {'1':5.3,'2':5,'3':4,'4':5.5,'5':4.3,'6':9.2,'7':10.8}
            eighty = {'1':8.9,'2':8.9,'3':7.8,'4':10.5,'5':8,'6':11.5,'7':11.5}
            ninty = {'1':12.1,'2':16.1,'3':16.8,'4':14.9,'5':8,'6':9.2,'7':10.8}
            nin_five = {'1':8.6,'2':16.1,'3':7.9,'4':4.8,'5':8,'6':9.2,'7':10.8}
            #time_start = {'60':0,'70':0,'80':0,'90':0,'95':18.6}
            time_start = {'60':sixty[str(case[i])],'70':seventy[str(case[i])],'80':eighty[str(case[i])],'90':ninty[str(case[i])],'95':nin_five[str(case[i])]}
            #time_end = {'60':sixty[str(case[i])],'70':seventy[str(case[i])],'80':eighty[str(case[i])],'90':ninty[str(case[i])],'95':nin_five[str(case[i])]}
            time_end = {'60':20,'70':20,'80':30.0,'90':25,'95':25}
            ...
            for time in np.arange(time_start[str(dir_list[a])],time_end[str(dir_list[a])],1):
                ...
                ex= [1,0.7,1.33,1.24,1.12,1,0.9,0.81,0.69] # expansion ratio
                ...
                ex_ratio = ex[1]  
                plt.title('$t = %.2f s$' %(0.04*(time-8.9) ))
                if ex_ratio in ex[:5]:
                    expansion = ex[0] + 0.08
                ...
                xx = 8+pow(2*delta_ratio*R_0*R_out - pow(delta_length,2),1/2)
                ...
                if y_min<0:
                    ...
                    y2 = math.fabs(y_min-delta_x)/(y_max - y_min)
                else:
                    y1 = 0
