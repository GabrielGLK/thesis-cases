...
case = [10,20,30,45,60,75,90]
a = 2
i = 0
while i < 7:  
    try:
        PATH_REFERENCE = os.path.abspath('/media/lguo/Elements/calculation/Taylor_bubble/2D/singularity/gradual/expansion/positions/{}/taylor_2D'.format(str(case[i])))
        ...
        PATH_particle = PATH_REFERENCE + '/curvature'
        ...
        PATH_COM = os.path.abspath("/media/lguo/Elements/writing/thesis-part/chapter-5/gradual/figure/expansion/particle/1")
        ...
        def get_particle(filename):
            fn = os.path.join(PATH_particle, filename)
            seg = []
            with open(fn, 'r') as file:
                for line in file.readlines():
                    line = line.rstrip('\n')
                    ...
                    if len(l) == 6:
                        #print float(l[0]), float(l[1])
                        seg.append([float(l[0]), float(l[1])-8, float(l[2]), float(l[3]), float(l[4]), float(l[5])])
            return seg
        ...
        def plot_particle(plt,sur,time,color,marker):
            ...
            for seg in sur:
                if seg[0] == time:
                    ...
                    y1 = seg[3]
                    y2 = seg[4]
                    plt.plot(x2,x1, linestyle=':', marker=marker, markersize = 5, color = color)
        ...
        def plot_velocity(plt, sur):
            ...
            ax.quiver(Y,X,V,U,M, units = 'xy',angles='xy',scale_units='xy', scale=20,cmap=plt.cm.jet,clim=(0,3))
            qq=ax.quiver(YN,X,VN,U,M,units = 'xy', angles='xy',scale_units='xy',scale=20,cmap=plt.cm.jet,clim=(0,3))
        ...
        def simulation(a):
            ...
            for time in np.arange(time_begin,time_end,0.5):
                ...
                sur1 = get_sur_1(time)
                sur2 = get_sur_2(time)
                B = get_particle('point-1')
                C = get_particle('point-2')
                A = get_particle('point-3')
                #plot_velocity(plt,sur2)
                a = out_data('out')
                
                ex= [1.1,1.2,1.3,1.45,1.5,1.6,1.7,1.8,1.9,2,2.5] # expansion ratio
                ...
                plot_particle(plt,A,time,"black",'o')
                plot_particle(plt,B,time,"brown",'x')
                plot_particle(plt,C,time,"green",'.')
                
                
                ex_ratio = ex[3]  
                a1 = 0.8
                b1 = -0.8
                #plt.title('$t = %.2f s$' %(0.04*(time- time_begin))) 
                if ex_ratio in ex[:]:
                    expansion = ex_ratio 
                    plt.xlim(b1,a1)
                else:
                ...
                y_min = -4
                y_max = 4
                plt.yticks((np.arange(int(y_min), y_max+0.5, step=1)))
                #plot_streamline(plt,sur2,a,time,ex,y_min,y_max,xh1,xh11)
                deg = [10,20,30,45,60,75,90]
                R_0 = 0.5
                R_ex = R_0*ex_ratio
                ...
                for a in np.arange(0,delta_x,0.01):
                    plt.axhspan(xmin = x1+a*math.tan(theta)/1.6,xmax = 1,ymin = 0,ymax = a,facecolor = 'w',alpha = 1)
                    plt.axhspan(xmin = 0,xmax = x4-a*math.tan(theta)/1.6,ymin = 0,ymax = a,facecolor = 'w',alpha = 1)
