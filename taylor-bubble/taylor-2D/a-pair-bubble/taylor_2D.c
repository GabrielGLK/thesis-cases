...
double geometry(double x, double y, double r) {

  coord center;
  foreach_dimension()
  center.x = 0;
  double distance = 4;
  double s = sphere (x-(length_ratio+distance)*r, y, center, r);
  double left = x - distance*r;
  double right = -x + (length_ratio+distance)*r;
  double top = -y + r;
  double bottom=0;
...
fraction (f, min(geometry(x,y,r_0),geometry(x-9*r_0,y,r_0)));
